...
class MainWindow:
    def __init__(self, root):
        self.root = root
        self.root.title("🎬 Windows 实时字幕捕获工具 v2.0")
    ...
    def setup_ui(self):
        ...
        ttk.Label(title_frame, text="🎬 Windows 实时字幕捕获工具 v2.0", style="Title.TLabel").pack(anchor=tk.W)
        ttk.Label(title_frame, text="专业多项目实时字幕转写工具", style="Status.TLabel").pack(anchor=tk.W, pady=(2, 0))
        ...
        self.editor_text_box.pack(fill=tk.BOTH, expand=True, pady=(0, 8))

        editor_btn_frame = ttk.Frame(editor_frame)
        editor_btn_frame.pack(fill=tk.X)
        
        self.copy_btn = ttkb.Button(editor_btn_frame, text="📋 复制全部内容", 
                                     command=self.copy_document_content, 
                                     bootstyle="info", width=16)
        self.copy_btn.pack(side=tk.RIGHT, padx=(4, 0))
        
        ttkb.Button(editor_btn_frame, text="💾 保存项目", command=self.save_current_project, 
                     bootstyle="success", width=14).pack(side=tk.RIGHT, padx=(4, 0))
        
        self.toggle_transcribe_btn = ttkb.Button(editor_btn_frame, text="⏸ 暂停转写", 
                                                  command=self.toggle_project_transcribe, 
                                                  bootstyle="warning", width=14)
        self.toggle_transcribe_btn.pack(side=tk.RIGHT)
        ...
        ttk.Label(bottom_frame, text="💡 提示：使用 Win+Ctrl+C 快捷键开启 Windows 实时字幕功能", 
